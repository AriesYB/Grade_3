...
int main(int argc, char const *argv[])
{
    ...
    unsigned char * p_map;
    int fd = open("/data/train_data.txt",O_RDONLY);
    if(fd=-1)
    {
	cout<<"打开文件失败"<<endl<<errno<<endl;
	exit(1);
    }
    p_map = (unsigned char*)mmap(NULL,4028,PROT_READ|PROT_WRITE,MAP_SHARED,fd,0);
    if(p_map = NULL)
    {
	cout<<"文件映射失败"<<endl;
    }
    ...
    l.readTrainData();
    l.readTestData();
    cout << "start train" << endl;    l.train(1);
    l.train(1);
